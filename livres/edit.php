...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if (empty($livre['code'])) {
        ...
        // Vérifier si le code existe déjà (sauf pour ce livre)
        $stmt = $conn->prepare("SELECT COUNT(*) FROM livres WHERE code = :code AND id != :id");
    ...
    if (empty($errors)) {
        try {
            $stmt = $conn->prepare("UPDATE livres SET titre = :titre, auteur = :auteur, code = :code, 
                                  date_publication = :date_publication, exemplaires = :exemplaires, image = :image 
                                  WHERE id = :id");
            ...
            <div class="col-md-8">
                <div class="card shadow">
                    <div class="card-header bg-primary text-white">
                        <h5 class="mb-0"><i class="fas fa-edit me-2"></i>Modifier un livre</h5>
                    </div>
                    <div class="card-body">
                        <?php if (!empty($errors)): ?>
